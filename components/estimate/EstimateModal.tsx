...
import { type EstimateCalculateResponse } from "@/hooks/useEstimateChat";
...
interface Props {
  result: EstimateCalculateResponse;
  nickname: string;
  onClose: () => void;
...
export default function EstimateModal({ result, nickname, onClose, onSave, initialSaved }: Props) {
  ...
  const {
    finalAmount, screenCount, baseDailyRate,
    step1BasicFee, uxMultiplier, step2UxFee,
    platformMultiplier, step3PlatformFee,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6">
      <div className="flex max-h-[calc(100vh-48px)] w-full max-w-xl flex-col rounded-2xl bg-white shadow-xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-8 pb-8">
          ...
          {/* 최종 금액 배너 */}
          <div
            className="flex justify-between items-center rounded-xl px-5 py-4 mb-6"
          ...
          <div className="rounded-xl bg-bg2 px-5 flex flex-col text-sm text-bodyfont2 mb-8">
            <div className="flex justify-between py-3 border-b border-gray-200">
              <span>기본 작업비</span>
              <span>
                {screenCount}화면 x {toMan(baseDailyRate).toLocaleString()}만 원 = {toMan(step1BasicFee).toLocaleString()}만 원
              </span>
            </div>
            <div className="flex justify-between py-3 border-b border-gray-200">
              <span>UX 기획 관여도</span>
